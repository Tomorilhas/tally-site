...
// Função para determinar a externa e cor do tally
void getExternaTally(size_t porta_idx, int &externa_num, const char* &cor_tally) {
    // Portas 1 e 2 -> externa 1, 3 e 4 -> externa 2, ..., 9 e 10 -> externa 5
    externa_num = (porta_idx / 2) + 1;
    // Impar: tally vermelho (PGM), par: tally verde (PW)
    if ((porta_idx % 2) == 0)
        cor_tally = "PGM"; // porta 1,3,5... (índice 0,2,4...)
    else
        cor_tally = "PW";  // porta 2,4,6... (índice 1,3,5...)
}
...
void publicarEstadoPorta(size_t idx, bool estado) {
    ...
    // Definição do status: circuito fechado = ativo, circuito aberto = BY (idle)
    const char *msg;
    if (estado == LOW) { // circuito fechado
        msg = cor_tally; // "PGM" ou "PW"
    } else {
        msg = "BY";      // circuito aberto = BY (idle)
    }
